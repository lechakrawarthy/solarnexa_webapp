...
export default function SNTreeCanvas() {
  ...
  useEffect(() => {
    ...
    let energyPts = []
    for (let i = 0; i < 3; i++) energyPts.push({ y: BASE - 170, progress: 0, delay: Math.floor(Math.random() * 60) })
    ...
    function drawPanel(p, opacity) {
      ...
      ctx.fillStyle = 'rgba(217,59,43,0.03)'
      ctx.beginPath(); roundRect(ctx, -p.w / 2, -p.h / 2, p.w, p.h / 2, 5); ctx.fill()
      ctx.restore()
    }
    ...
    function draw() {
      ...
      t++
      if (branchProgress < 1) branchProgress += 0.018
      if (branchProgress > 0.4 && panelOpacity < 1) panelOpacity += 0.025

      // Sun
      const sunPulse = 1 + Math.sin(t * 0.04) * 0.04
      ctx.save()
      ctx.globalAlpha = 0.06 + Math.sin(t * 0.03) * 0.02
      ctx.beginPath(); ctx.arc(SUN.x, SUN.y, SUN.r * 2.2, 0, Math.PI * 2)
      ...
      if (panelOpacity > 0.1) {
        panels.forEach(p => {
          ctx.save(); ctx.globalAlpha = panelOpacity * 0.4
      ...
      // Ground shadow
      ctx.save()
      ctx.globalAlpha = panelOpacity * 0.5
      const grd = ctx.createRadialGradient(CX, BASE + 20, 0, CX, BASE + 20, 120)
      grd.addColorStop(0, 'rgba(28,24,20,0.12)'); grd.addColorStop(1, 'transparent')
      ctx.fillStyle = grd; ctx.beginPath(); ctx.ellipse(CX, BASE + 20, 120, 12, 0, 0, Math.PI * 2); ctx.fill()
      ctx.restore()
      ...
      if (panelOpacity > 0.4) {
        ...
        ;[EV.x + 16, EV.x + EV.w - 16].forEach(px => {
          ctx.beginPath(); ctx.arc(px, EV.y + EV.h / 2, 5, 0, Math.PI * 2)
          ctx.fillStyle = 'rgba(217,59,43,0.12)'; ctx.strokeStyle = 'rgba(217,59,43,0.4)'; ctx.lineWidth = 0.7
          ctx.fill(); ctx.stroke()
        })
        ctx.fillStyle = 'rgba(217,59,43,0.06)'; ctx.strokeStyle = 'rgba(217,59,43,0.2)'; ctx.lineWidth = 0.5
        ctx.beginPath(); roundRect(ctx, CX - 10, EV.y + 5, 20, 16, 3); ctx.fill(); ctx.stroke()
        ctx.restore()
      }
      ...
      if (panelOpacity > 0.6) {
        energyPts.forEach((p, i) => {
          ...
          p.progress += 0.008
          if (p.progress >= 1) { p.progress = 0; p.delay = Math.floor(Math.random() * 80 + 20) }
          const py = BASE - 170 + p.progress * (BASE - (BASE - 170))
          const pAlpha = Math.sin(p.progress * Math.PI) * (panelOpacity - 0.6) / 0.4
          ctx.save(); ctx.globalAlpha = pAlpha * 0.8
          ctx.beginPath(); ctx.arc(CX + (i - 1) * 3, py, 2.5, 0, Math.PI * 2)
          ctx.fillStyle = PLASMA; ctx.fill()
          ctx.restore()
        })
      }
      ...
      if (panelOpacity > 0.7) {
        ...
        panels.slice(0, 3).forEach((p, i) => {
          ...
          if (localPT < 0.5) {
            const r = localPT * 2 * 22
            const a = (0.5 - localPT) * 2 * pulseAlpha * 0.25
            ctx.save(); ctx.globalAlpha = a
            ctx.strokeStyle = PLASMA; ctx.lineWidth = 0.8
